...
function ft_breadcrumbs() {
  ...
  if ( !is_home() && !is_front_page() || is_paged() ) {
    ...
    global $post;
    $homeLink = home_url();
    echo ' <span typeof="v:Breadcrumb"><a property="v:title" rel="v:url" href="' . $homeLink . '">' . __( 'Homepage' , 'ft' ) . '</a></span> ' . $delimiter . ' ';
    ...
    if ( is_category() ) {
      ...
      echo $before ;
	  printf( __( 'Search Results for: %s', 'ft' ),  get_search_query() );
	  echo  $after;
 
    } elseif ( is_tag() ) {
	  echo $before ;
	  printf( __( 'Tag Archives: %s', 'ft' ), single_tag_title( '', false ) );
	  echo  $after;
      ...
      echo $before ;
	  printf( __( 'Author Archives: %s', 'ft' ),  $userdata->display_name );
	  echo  $after;
    ...
    if ( get_query_var('paged') ) {
      ...
      echo __('page ', 'ft') . ' ' . get_query_var('paged');
      if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
    }
